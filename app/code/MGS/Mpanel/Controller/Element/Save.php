    ...
    public function execute()
    {
		if($this->customerSession->getUsePanel() == 1){
			...
			switch ($data['type']) {
				...
				/* Instagram Block */
				case "instagram":
					$dataInit = ['link', 'like', 'use_slider', 'autoplay', 'stop_auto', 'navigation', 'pagination'];
					$data = $this->reInitData($data, $dataInit);
					$content = '{{block class="MGS\Social\Block\Panel\Widget" mgs_panel_title="'.$this->encodeHtml($data['setting']['title']).'" mgs_panel_note="'.$this->encodeHtml($data['setting']['additional_content']).'" limit="'.$data['setting']['limit'].'" resolution="'.$data['setting']['resolution'].'" link="'.$data['setting']['link'].'" perrow="'.$data['setting']['perrow'].'" use_slider="'.$data['setting']['use_slider'].'" like="'.$data['setting']['like'].'"';
